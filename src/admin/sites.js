...
function AdminSites() {
  ...
  const CreateNewSite = async () => {
    ...
    if (!sitename.trim() || !projectname.trim() || !clientname.trim() || !sd.trim()|| !contact.trim() || !address.trim() || !standard.trim()) {
      alert('Please fill in all fields');
      setLoader(false);
      return;
    }

    const url = `${process.env.REACT_APP_BASE_URL}add_site.php?sitename=${sitename}&projectname=${projectname}&address=${address}&clientname=${clientname}&contact=${contact}&sd=${sd}&standard=${standard}&adminid=${localStorage.getItem('adminid')}`;
      ...
      {CreateSite && (
        <div className="popup_user_back">
          <div className="popup_user">
            ...
            <input ref={standardRef} type="number" placeholder="Standard" />
